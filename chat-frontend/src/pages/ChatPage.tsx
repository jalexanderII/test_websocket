...
import type { APIChat, Chat, JsonValue, Message } from "@/types";
import { atom, useAtom } from "jotai";
import { Home, Loader2, Plus, Send, Trash2, X } from "lucide-react";
...
export default function ChatPage() {
	...
	const {
		...
		readyState,
		getWebSocket,
	} = useWebSocket(`ws://localhost:8005/api/ws/${userId}`, {
		onMessage: (event) => {
						...
						setMessages((prev) => {
							...
							if (structuredId) {
								const existingMessageIndex = prev.findIndex(
									(msg) =>
										msg.structured &&
										msg.metadata?.structured_id === structuredId,
								);

								if (existingMessageIndex !== -1) {
									console.log(
										"Updating existing structured message at index:",
	...
	const loadChat = useCallback(
		async (chatId: number) => {
			try {
				...
				const response = await fetch(
					`http://localhost:8005/api/chats/${chatId}`,
				);
				...
				if (connected) {
					console.log(
						"[WebSocket] Sending join_chat message for chat:",
						chatId,
					);
					...
					const retryInterval = setInterval(() => {
						if (connected) {
							console.log(
								"[WebSocket] Connection established, retrying join chat",
							);
							sendWebSocketMessage(
								JSON.stringify({
									action: "join_chat",
	...
	const sendMessage = () => {
		console.log(
			"[Chat] Attempting to send message. Connected:",
			connected,
			"CurrentChatId:",
			currentChatId,
		);
	...
	const cleanupEmptyChats = useCallback(async () => {
		try {
			const response = await fetch(
				"http://localhost:8005/api/users/1/chats/empty",
				{
					method: "DELETE",
				},
			);
			...
			const pingInterval = setInterval(() => {
				if (wsRef.current?.readyState === WebSocket.OPEN) {
					...
					if (timeSinceLastPong > 30000) {
						console.warn(
							`No messages received in ${Math.round(timeSinceLastPong / 1000)}s, marking connection as unhealthy`,
						);
						setConnectionHealth("unhealthy");
					} else if (
						connectionHealth === "unhealthy" &&
						timeSinceLastPong < 30000
					) {
						setConnectionHealth("healthy");
					}
				}
