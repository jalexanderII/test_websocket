...
class ChatService:
    ...
    def _is_chat_empty(self, chat_id: int) -> bool:
        ...
        message_count = self.db.query(MessageDB).filter(MessageDB.chat_id == chat_id).count()
    ...
    def _get_latest_empty_chat(self, user_id: int) -> Optional[Chat]:
        ...
        db_chats = self.db.query(ChatDB).filter(ChatDB.user_id == user_id).order_by(ChatDB.created_at.desc()).all()
    ...
    async def stream_ai_response(
        ...
        chat_id: int,
        user_message: str,
        interim_message_handler: Optional[Callable[[str], Coroutine[Any, Any, None]]] = None,
    ...
    async def stream_structured_ai_response(
        ...
        try:
            async for chunk in self.adapter.stream_structured_response(user_message, response_model, history=history):
